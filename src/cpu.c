...
void cpu_clock(_state* state) {
    ...
    uint8_t opcode = bus_read(state, state->pc++);
...
void cpu_reset(_state* state) {
    uint16_t low = bus_read(state, RST_VECTOR);
    uint16_t high = bus_read(state, RST_VECTOR + 1);
    state->pc = (high << 8) | low;
...
void cpu_irq(_state* state) {
    ...
    bus_write(state, 0x0100 + state->stack--, (state->pc >> 8) & 0xFF);
    bus_write(state, 0x0100 + state->stack--, state->pc & 0xFF);

    set_flag(state, B, 0);
    set_flag(state, U, 1);
    set_flag(state, I, 1);
    bus_write(state, 0x0100 + state->stack--, state->status);

    uint16_t low = bus_read(state, IRQ_VECTOR);
    uint16_t high = bus_read(state, IRQ_VECTOR + 1);
    state->pc = (high << 8) | low;
...
void cpu_nmi(_state* state) {
    ...
    uint16_t low = bus_read(state, NMI_VECTOR);
    uint16_t high = bus_read(state, NMI_VECTOR + 1);
    state->pc = (high << 8) | low;
...
uint8_t am_zpg(_state* state) {
    state->addr = bus_read(state, state->pc++);
    return 0;
}

uint8_t am_zpx(_state* state) {
    state->addr = (bus_read(state, state->pc++) + state->rx) & 0xFF;
    return 0;
}

uint8_t am_zpy(_state* state) {
    state->addr = (bus_read(state, state->pc++) + state->ry) & 0xFF;
    return 0;
}

uint8_t am_abs(_state* state) {
    uint16_t low = bus_read(state, state->pc++);
    uint16_t high = bus_read(state, state->pc++);
...
uint8_t am_idr(_state* state) {
    uint16_t p_low = bus_read(state, state->pc++);
    uint16_t p_high = bus_read(state, state->pc++);
    uint16_t ptr = (p_high << 8) | p_low;

    uint16_t high_addr = (p_low == 0xFF) ? (ptr & 0xFF00) : (ptr + 1);
    state->addr = (bus_read(state, high_addr) << 8) | bus_read(state, ptr);

    return 0;
}

uint8_t am_idx(_state* state) {
    uint16_t base = bus_read(state, state->pc++);
    uint16_t low = bus_read(state, (base + state->rx) & 0x00FF);
    uint16_t high = bus_read(state, (base + state->rx + 1) & 0x00FF);
...
uint8_t am_idy(_state* state) {
    ...
    uint16_t low = bus_read(state, base & 0x00FF);
    uint16_t high = bus_read(state, (base + 1) & 0x00FF);
    state->addr = ((high << 8) | low) + state->ry;
...
uint8_t op_adc(_state* state) {
    uint8_t memory = bus_fetch(state);
...
uint8_t op_asl(_state* state) {
    ...
    if (is_imp(state)) {
        ...
        bus_write(state, state->addr, res & 0xFF);
    }
...
uint8_t op_dec(_state* state) {
    uint8_t memory = bus_fetch(state);
    memory--;
    bus_write(state, state->addr, memory);
...
uint8_t op_lsr(_state* state) {
    ...
    if (is_imp(state)) {
        ...
        bus_write(state, state->addr, res);
    }
...
uint8_t op_pha(_state* state) {
    bus_write(state, 0x0100 + state->stack--, state->ra);

	return 0;
}

uint8_t op_php(_state* state) {
    bus_write(state, 0x0100 + state->stack--, state->status | B);

	return 0;
}

uint8_t op_pla(_state* state) {
    state->ra = bus_read(state, 0x0100 + ++state->stack);
...
uint8_t op_plp(_state* state) {
    state->status = bus_read(state, 0x0100 + ++state->stack);
...
uint8_t op_rti(_state* state) {
    state->status = bus_read(state, 0x0100 + ++state->stack);
    state->pc = bus_read(state, 0x0100 + ++state->stack);
    state->pc |= (uint16_t)bus_read(state, 0x0100 + ++state->stack) << 8;

	return 0;
}

uint8_t op_rts(_state* state) {
    state->pc = bus_read(state, 0x0100 + ++state->stack) + 1;
    state->pc |= (uint16_t)bus_read(state, 0x0100 + ++state->stack) << 8;
...
uint8_t op_sta(_state* state) {
    bus_write(state, state->addr, state->ra);
	return 0;
}

uint8_t op_stx(_state* state) {
    bus_write(state, state->addr, state->rx);
	return 0;
}

uint8_t op_sty(_state* state) {
    bus_write(state, state->addr, state->ry);
	return 0;
}
